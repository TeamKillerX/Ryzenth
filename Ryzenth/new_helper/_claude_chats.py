...
class ChatsClaudeAsync:
    ...
    def _get_client(self) -> RyzenthApiClient:
        if self._client is None:
            api_key = getattr(
                self.parent,
                "_api_key",
                None) or os.environ.get("ANTHROPIC_API_KEY")

            if not api_key or not isinstance(
                    api_key, str) or not api_key.strip():
                raise AuthenticationError(
                    "Missing or invalid API key for Claude client initialization.")
            try:
    ...
    async def ask(
        ...
        *,
        ...
        max_tokens: Union[int, float] = 1000
    ) -> ResponseResult:
        ...
        try:
            ...
            raise InternalServerClaudeError(f"Claude chats failed: {e}") from e
        finally:
            pass
