...
class ChatsZaiAsync:
    ...
    def _get_client(self) -> RyzenthApiClient:
        if self._client is None:
            api_key = getattr(self.parent, "_api_key", None) or os.environ.get("ZAI_API_KEY")

            if not api_key or not isinstance(api_key, str) or not api_key.strip():
                raise WhatFuckError("Missing or invalid API key for Zai client initialization.")
            try:
    ...
    async def ask(
        ...
        *,
        ...
        model: str = "glm-4.5",
        max_tokens: Union[int, float] = 1024,
        temperature: Union[int, float] = 0.6,
        stream: bool = False
    ) -> ResponseResult:
