...
class ChatsHuggingFaceAsync:
    ...
    def _get_client(self) -> RyzenthApiClient:
        if self._client is None:
            api_key = getattr(
                self.parent,
                "_api_key",
                None) or os.environ.get("HUGGING_API_KEY")
    ...
    async def ask(
        ...
        *,
        ...
        provider_server: str = "fireworks-ai",
        ...
        try:
            ...
            self.logger.error(f"Hugging Face chats failed: {e}")
            raise InternalServerChatsHuggingFaceError(f"Hugging Face failed: {e}") from e
        finally:
            pass
